...
* `verification_otp`: VARCHAR(4) [Bổ sung: Mã OTP 4 số dùng cho cả xác thực email và quên mật khẩu]
...
* `password_changed_at`: DATETIME [Bổ sung: Thời điểm đổi mật khẩu để thu hồi token cũ]
